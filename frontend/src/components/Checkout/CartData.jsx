...
const CartData = ({
  ...
  // Tính giảm giá sản phẩm và phí ship riêng biệt (chỉ để hiển thị)
  ...
  // Tính tổng tiền cho mỗi shop (subtotal + shipping - discount)
  const totalPerShop = Object.fromEntries(
    Object.entries(cartByShop).map(([shopId, shopData]) => [
      shopId,
      shopData.subtotal +
        (shippingPerShop[shopId] || 0) -
        (discountPricePerShop[shopId] || 0),
    ])
  );

  console.log("Product Discount Per Shop:", productDiscountPerShop);
  console.log("Shipping Discount Per Shop:", shippingDiscountPerShop);
  console.log("Total Per Shop:", totalPerShop);
  ...
  return (
    <div className="w-full bg-[#fff] rounded-md p-5 pb-8">
        ...
        <div key={shopId} className="mb-6 border-b pb-4">
          ...
          {shopData.items.map((item, index) => (
            <div key={index} className="flex items-center mb-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-[12px] text-gray-600">
                  Price: {(item.discountPrice * item.qty).toLocaleString("vi-VN")}{" "}
                  VNĐ
                </p>
              </div>
            </div>
          ...
          <div className="mb-4">
            <button
              ...
              onClick={() => openCouponSelector(shopId)}
            >
              Select Coupon
            </button>
      ...
      {Object.values(productDiscountPerShop).reduce((acc, p) => acc + p, 0) >
        0 && (
        <div className="flex justify-between mb-2">
          <h3 className="text-[16px] font-[400] text-[#000000a4]">
          ...
          <h5 className="text-[18px] font-[600] text-green-600">
            -{" "}
            {Object.values(productDiscountPerShop)
      ...
      {Object.values(shippingDiscountPerShop).reduce((acc, p) => acc + p, 0) >
        0 && (
        <div className="flex justify-between mb-2">
          <h3 className="text-[16px] font-[400] text-[#000000a4]">
          ...
          <h5 className="text-[18px] font-[600] text-green-600">
            -{" "}
            {Object.values(shippingDiscountPerShop)
              .reduce((acc, p) => acc + p, 0)
              .toLocaleString("vi-VN")}{" "}
            VNĐ
          </h5>
