...
const DashboardMessages = () => {
    ...
    const getConversation = async () => {
      try {
        const resonse = await axios.get(
          `${process.env.REACT_APP_SERVER}/conversation/get-all-conversation-shipper/${shipper?._id}`,
          {
  ...
  const onlineCheck = (chat) => {
    const chatMembers = chat.members.find((member) => member !== shipper?._id);
  ...
  const sendMessageHandler = async (e) => {
    ...
    const receiverId = currentChat.members.find(
      (member) => member.id !== shipper._id
    );

    socketId.emit("sendMessage", {
      senderId: shipper._id,
      receiverId,
      text: newMessage,
    });
  ...
  const imageSendingHandler = async (e) => {
    ...
    formData.append("sender", shipper._id);
    formData.append("text", newMessage);
    formData.append("conversationId", currentChat._id);

    const receiverId = currentChat.members.find(
      (member) => member !== shipper._id
    );
            ...
            return (
              <div
                className={`flex w-full my-2 ${
                  item.sender === shipperId ? "justify-end" : "justify-start"
                }`}
                ref={scrollRef}
              >
                {item.sender !== shipperId && (
                  <img
                    src={userData?.avatar}
                  ...
                  <div>
                    <div
                      className={`w-max p-2 rounded ${
                        item.sender === shipperId ? "bg-[#000]" : "bg-[#38c776]"
                      } text-[#fff] h-min`}
                    >
                      <p>{item.text}</p>
                    </div>
