...
import { uploadFileRequest } from "../../request/api"; // Import uploadFileRequest giống Signup.jsx
...
const ShopCreate = () => {
  ...
  const handleFileInputChange = (e) => {
    ...
    if (file) {
      // Validate file type and size giống Signup.jsx
  ...
  const handleSubmit = async (e) => {
    ...
    if (!email || !password || !name.trim()) {
      toast.error("Vui lòng điền đầy đủ email, tên shop và mật khẩu");
      setLoading(false);
      return;
    }

    const newForm = new FormData();
    if (avatar) newForm.append("file", avatar); // Chỉ append nếu avatar tồn tại
    newForm.append("email", email.trim());
    newForm.append("name", name.trim());
    newForm.append("password", password);

    // Log FormData để kiểm tra
    console.log("Submitting shop creation with:");
    for (let [key, value] of newForm.entries()) {
      console.log(`${key}: ${value}`);
    }

    try {
      const res = await uploadFileRequest("/shop/create-shop", newForm); // Sử dụng uploadFileRequest
